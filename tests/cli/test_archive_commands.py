...
class TestArchiveCommands:
    ...
    def test_archive_help(self):
        ...
        assert "Archive experiments by moving them to archived directory" in result.output
    ...
    def test_unarchive_help(self):
        ...
        assert result.exit_code == 0
        assert "Unarchive experiments by moving them back to experiments directory" in result.output
        assert "--status" in result.output
    ...
    def test_unarchive_nonexistent_experiment(self):
        ...
        assert result.exit_code == 1
        assert "No archived experiment found with ID or name 'nonexistent123'" in result.output
    ...
    def test_archive_with_filters_no_matches(self):
        ...
        result = self.runner.invoke(cli, ["archive", "--name", "definitely-nonexistent-*", "--force"])
        assert result.exit_code == 0
    ...
    def test_delete_with_filters_no_matches(self):
        ...
        # Use a name pattern that won't match any experiments
        result = self.runner.invoke(cli, ["delete", "--name", "definitely-nonexistent-*", "--force"])
        assert result.exit_code == 0
    ...
    def test_unarchive_with_filters_no_matches(self):
        ...
        # Use a name pattern that won't match any experiments
        result = self.runner.invoke(cli, ["unarchive", "--name", "definitely-nonexistent-*", "--force"])
        assert result.exit_code == 0

...
class TestUpdateCommand:
    ...
    def test_update_help(self):
        ...
        assert "Update experiment metadata including name, description, status, and tags" in result.output
    ...
    def test_update_mutual_exclusivity_error(self):
        ...
        result = self.runner.invoke(cli, ["update", "exp1", "--status", "completed", "--set-name", "test"])
    ...
    def test_update_nonexistent_experiment(self):
        ...
        result = self.runner.invoke(cli, ["update", "nonexistent123", "--set-name", "test"])
        assert result.exit_code == 1
        assert "No regular experiment found with ID or name 'nonexistent123'" in result.output

    def test_update_with_filters_no_matches(self):
        """Test update command with filters that match no experiments."""
        result = self.runner.invoke(cli, ["update", "--name", "definitely-nonexistent-*", "--set-name", "test", "--force"])
        assert result.exit_code == 0
    ...
    def test_update_with_valid_identifiers_only(self):
        ...
        assert "Cannot use both experiment identifiers and filter options" not in result.output
    ...
    def test_update_invalid_status_value(self):
        ...
        result = self.runner.invoke(cli, ["update", "exp1", "--set-status", "invalid-status"])
    ...
    def test_update_dry_run_with_identifiers(self):
        ...
        result = self.runner.invoke(cli, ["update", "nonexistent123", "--set-name", "test", "--dry-run"])
        assert result.exit_code == 1
    ...
    def test_update_dry_run_with_filters(self):
        ...
        result = self.runner.invoke(cli, ["update", "--name", "definitely-nonexistent-*", "--set-name", "test", "--dry-run"])
        assert result.exit_code == 0
    ...
    def test_update_archived_flag(self):
        ...
        result = self.runner.invoke(cli, ["update", "exp1", "--set-name", "test", "--archived"])
        assert result.exit_code == 1
    ...
    def test_update_multiple_tag_operations(self):
        ...
        result = self.runner.invoke(cli, ["update", "exp1", "--add-tag", "tag1", "--add-tag", "tag2", "--remove-tag", "old-tag"])
        assert result.exit_code == 1
        # Should not show mutual exclusivity error, should show experiment not found
        assert "Cannot use both experiment identifiers and filter options" not in result.output
    ...
    def test_update_clear_fields(self):
        ...
        result = self.runner.invoke(cli, ["update", "exp1", "--set-name", "", "--set-description", ""])
        assert result.exit_code == 1
        # Should not show mutual exclusivity error, should show experiment not found
        assert "Cannot use both experiment identifiers and filter options" not in result.output
        assert "No regular experiment found" in result.output
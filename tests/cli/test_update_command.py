...
class TestUpdateCommand:
    ...
    def test_update_help_contains_required_sections(self):
        ...
        expected_options = [
            "--set-name", "--set-description", "--set-status",
            "--add-tag", "--remove-tag", "--dry-run", "--archived"
        ]
    ...
    def test_update_invalid_status_values(self, invalid_status):
        ...
        result = self.runner.invoke(cli, ["update", "exp1", "--set-status", invalid_status])
    ...
    def test_update_nonexistent_experiments(self, experiment_id):
        ...
        result = self.runner.invoke(cli, ["update", experiment_id, "--set-name", "test"])
        
        assert result.exit_code == 1
        assert f"No regular experiment found with ID or name '{experiment_id}'" in result.output
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (["--name", "nonexistent-*experiment*"], ["--set-description", "batch updated"]),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ["update", "--name", "definitely-nonexistent-unique-pattern-*", "--set-name", "test", "--dry-run"],
    ...
    def test_update_comprehensive_operation_combination(self):
        ...
        command_args = [
            "update", "test-exp",
            "--set-name", "comprehensive-test",
            "--set-description", "Updated with comprehensive test",
    ...
    def test_update_with_complex_filter_combinations(self, complex_filter_combination):
        ...
        command_args = ["update"] + complex_filter_combination + ["--set-name", "filtered-update", "--force"]
        result = self.runner.invoke(cli, command_args)
    ...
    def test_update_command_force_flag_behavior(self):
        ...
        result = self.runner.invoke(cli, [
            "update", "--name", "nonexistent-*", "--set-name", "test", "--force"
        ])

...
def confirm_experiment_operation(
    ...
    click.echo(f"The following {len(experiments)} experiment(s) will be {operation_verb}:")
    click.echo()
...
def find_experiments_by_identifiers(
    ...
    for identifier in identifiers:
        if archived_only:
            ...
            if result is None:
                raise click.ClickException(f"No archived experiment found with ID or name '{identifier}'")
            ...
            if isinstance(result, list):
                ...
                if not archived_results:
                    raise click.ClickException(f"No archived experiment found with name '{identifier}'")
                elif len(archived_results) == 1:
                    result = archived_results[0]
                else:
                    # Multiple archived experiments with same name
                    click.echo(f"Multiple archived experiments found with name '{identifier}':")
                    click.echo()
                    
                    formatter = ExperimentTableFormatter()
                    formatter.print_experiments_table(archived_results)
                    click.echo()
                    click.echo("Please use specific experiment IDs instead of names for bulk operations.")
                    raise click.ClickException(f"Ambiguous archived experiment name: '{identifier}'")
            else:
                # Single result - check if it's archived
                if not result.get("archived", False):
                    raise click.ClickException(f"Experiment '{identifier}' is not archived")
        else:
            ...
            if result is None:
                ...
                raise click.ClickException(f"No {location} experiment found with ID or name '{identifier}'")

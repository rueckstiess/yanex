...
@click.option(
    ...
    "tags",
    multiple=True,
    help="Archive experiments with ALL specified tags"
)
@click.option(
    "--started-after",
...
@click.option(
    "--started-before", 
    help="Archive experiments started before date/time"
)
@click.option(
    "--ended-after",
    help="Archive experiments ended after date/time"
)
@click.option(
    "--ended-before",
    help="Archive experiments ended before date/time"
)
@click.option(
    "--force",
    is_flag=True,
...
def archive_experiments(
    ...
    try:
        ...
        if not confirm_experiment_operation(experiments, "archive", force, default_yes=True):
        ...
        for exp in experiments:
            try:
                ...
                exp_name = exp.get("name", "[unnamed]")
                click.echo(f"  ✗ Failed to archive {experiment_id} ({exp_name}): {e}", err=True)
        ...
        if success_count == len(experiments):
            ...
            click.echo(f"Archived {success_count} experiment(s), {failed_count} failed.", err=True)
            ctx.exit(1)

...
@click.option(
    "--status",
    type=click.Choice(["created", "running", "completed", "failed", "cancelled"], case_sensitive=False),
    ...
    try:
        ...
        if started_spec:
            ...
            if started_after is None:
                click.echo(f"Error: Could not parse start time '{started_spec}'", err=True)
                click.echo("Examples: 'today', 'last week', '2 hours ago', '2023-01-01'", err=True)
                raise click.Abort()
        ...
        if not experiments:
            ...
            _show_filter_suggestions(status, name_pattern, tags, started_spec, ended_spec)
            return

...
@click.option(
    "--ended-after",
    help="Filter experiments ended after date/time for bulk updates"
)
@click.option(
    "--ended-before",
    help="Filter experiments ended before date/time for bulk updates"
)
@click.option(
    "--set-name",
    "new_name",
    help="Set experiment name (use empty string to clear)"
)
@click.option(
    "--set-description",
    "new_description", 
...
@click.option(
    ...
    "add_tags",
    multiple=True,
    help="Add tag to experiment(s) (repeatable)"
)
@click.option(
    "--remove-tag",
    "remove_tags",
...
@click.option(
    "--archived",
    is_flag=True,
    help="Update archived experiments"
)
@click.option(
    "--force",
    is_flag=True,
...
def update_experiments(
    ...
    try:
        ...
        if not any([new_name is not None, new_description is not None, new_status, add_tags, remove_tags]):
            click.echo("Error: Must specify at least one update option (--set-name, --set-description, --set-status, --add-tag, --remove-tag)", err=True)
            ctx.exit(1)
        
        # Validate mutually exclusive targeting
        has_identifiers = len(experiment_identifiers) > 0
        has_filters = any([filter_status, filter_name_pattern, filter_tags, started_after, started_before, ended_after, ended_before])
        ...
        if len(experiments) > 1 or dry_run:
            if not confirm_experiment_operation(experiments, "update", force or dry_run, "updated"):
        ...
        for exp in experiments:
            try:
                ...
                exp_name = exp.get("name", "[unnamed]")
                click.echo(f"  ✗ Failed to update {experiment_id} ({exp_name}): {e}", err=True)
        ...
        if success_count == len(experiments):
            ...
            click.echo(f"Updated {success_count} experiment(s), {failed_count} failed.", err=True)
            ctx.exit(1)

...
def show_experiment(ctx, experiment_identifier: str, show_metrics: Optional[str], archived: bool):
    ...
    try:
        ...
        if isinstance(experiment, list):
            click.echo(f"Multiple experiments found with name '{experiment_identifier}':")
            click.echo()
            
            # Show a filtered list using the existing list formatter
            formatter.print_experiments_table(experiment)
            click.echo()
            click.echo("Please use the specific experiment ID with 'yanex show <id>' to view details.")
            ctx.exit(1)
...
def display_experiment_details(manager: ExperimentManager, experiment: Dict[str, Any], formatter: ExperimentTableFormatter, requested_metrics: Optional[List[str]] = None, include_archived: bool = False):
    ...
    if end_time:
        end_label = "Completed" if completed_at else ("Failed" if failed_at else "Cancelled")
    ...
    if tags or description:
        ...
        console.print(Panel(info_table, title="[bold]Experiment Info[/bold]", box=box.ROUNDED))
        console.print()
    ...
    try:
        ...
        if config:
            ...
            console.print(Panel(config_table, title="[bold]Configuration[/bold]", box=box.ROUNDED))
            console.print()
    except Exception:
        ...
        if results:
            ...
            if requested_metrics:
                ...
                if missing_metrics:
                    ...
                    warning_text.append(f"Requested metrics not found: {', '.join(missing_metrics)}", style="yellow")
                    console.print(warning_text)
                
                if not shown_metrics:
                    console.print(Text("No requested metrics found in experiment results.", style="red"))
                    return
                ...
                key_metrics = ["accuracy", "loss", "epoch", "learning_rate", "f1_score", "precision", "recall"]
                shown_metrics = []
                ...
                if other_count > 0:
                    results_table.add_column(f"(+{other_count} more)", justify="center", style="dim")
    ...
    try:
        ...
        if artifacts_dir.exists():
            ...
            if artifacts:
                artifacts_table = Table(show_header=True, header_style="bold magenta", box=box.SIMPLE)
                ...
                console.print(Panel(artifacts_table, title="[bold]Artifacts[/bold]", box=box.ROUNDED))
                console.print()
    except Exception:
    ...
    try:
        ...
        if env_info or git_info:
            ...
            if git_info:
                ...
                commit_hash = git_info.get("commit_hash_short", git_info.get("commit_hash", "unknown"))
            ...
            if python_info:
                ...
                if python_version != "unknown" and "(" in python_version:
                    python_version = python_version.split(" (")[0]  # e.g., "3.11.9" from "3.11.9 (main, ...)"
            ...
            console.print(Panel(env_table, title="[bold]Environment[/bold]", box=box.ROUNDED))
            console.print()
    except Exception:

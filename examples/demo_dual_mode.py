...
def main():
    ...
    print(f"   learning_rate: {learning_rate} {'(from yanex)' if not experiment.is_standalone() else '(default)'}")
    print(f"   epochs:        {epochs} {'(from yanex)' if not experiment.is_standalone() else '(default)'}")
    print(f"   model_type:    {model_type} {'(from yanex)' if not experiment.is_standalone() else '(default)'}")
    print(f"   batch_size:    {batch_size} {'(from yanex)' if not experiment.is_standalone() else '(default)'}")
...
"""
    ...
    if experiment.is_standalone():
        ...
        print(f"   💾 Results saved to: ~/.yanex/experiments/{experiment.get_experiment_id()}")
